...
/**
 * TOCHATBE_Admin_Woo_Order_Chat class.
 */
class TOCHATBE_Admin_Woo_Order_Chat {
	...
	public function admin_order_data_after_billing_address( $order ) {
		...
		<div
			...
			data-tochat-order-billing-number="<?php echo esc_attr( $phone_number ); // WPCS: XSS ok. ?>"
			data-tochat-order-message="<?php echo esc_textarea( $order_message ); ?>"
			data-tochat-order-id="<?php echo absint( $order->get_id() ); ?>"
		>
		...
		<p><?php echo wp_kses_post( $this->get_last_order_message( $order->get_id() ) ); ?></p>
		<?php
	}
	...
	public function chat_popup() {
		...
		<div class="tochatbe-woo-order-popup">
			<form action="#" method="post">
				<input type="hidden" name="order_id" value="<?php echo absint( get_the_ID() ); ?>">
				<div>
	...
	public function add_order_column_content( $column ) {
		...
		if ( 'tochatbe_contact' === $column ) {
			...
			<div
				...
				data-tochat-order-billing-number="<?php echo esc_attr( $this->get_phone_number( $order ) ); ?>"
				data-tochat-order-message="<?php echo esc_textarea( $order_message ); ?>"
				data-tochat-order-id="<?php echo absint( $order->get_id() ); ?>"
			>
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
			...
			<p style="max-width: 300px"><?php echo wp_kses_post( $this->get_last_order_message( $post->ID ) ); ?></p>
			<?php
		}
	}
					...
					<?php foreach ( $orders as $order ) : ?>
						...
						<tr>
							...
							<td>
								<div
									class="tochatbe-woo-order-btn"
									data-tochat-order-billing-number="<?php echo esc_attr( $this->get_phone_number( $order ) ); // WPCS: XSS ok. ?>"
									data-tochat-order-message="<?php echo esc_textarea( $order_message ); ?>"
									data-tochat-order-id="<?php echo absint( $order->get_id() ); ?>"
								>
									<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
	...
	protected function get_last_order_message( $order_id ) {
		...
		foreach ( $notes as $note ) {
			...
			$time_in_ago = human_time_diff( $timestamp, current_time( 'U' ) ); // phpcs:ignore WordPress.DateTime.CurrentTimeTimestamp.Requested
	...
	protected function get_private_order_notes( $order_id ) {
		...
		$results        = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM `$table_perfixed` WHERE comment_post_ID = %d AND comment_type LIKE 'order_note' ORDER BY comment_ID DESC", // phpcs:ignore
				$order_id
			)
		);
